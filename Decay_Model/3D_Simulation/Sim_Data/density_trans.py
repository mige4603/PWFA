# -*- coding: utf-8 -*-
"""
Created on Tue Jun 18 17:45:37 2019
...
ax2.set_ylim(0, 3)
ax2.plot([x_dom[0], x_dom[-1]], [ax2_y_dom[half_pts], ax2_y_dom[half_pts]], color=color, ls='--',  label='line out path')
...
for d in range(svar.dumps):
    ...
    den = myFile['{0}/Density/time_{1}'.format(svar.fluid, d)][0::, 0::, den_z_ind] 
    #print(den.shape)
    #print(x_dom_og.shape)
    ...
    image = ax1.imshow(im, extent=(x_dom[0], x_dom[-1], x_dom[0], x_dom[-1]), vmin=0, vmax=1, animated=True)
    image1 = image
    
    if d == 0:
        line1, = ax2.plot(x_dom, line, color=color, label='line out')
    else:
        line1, = ax2.plot(x_dom, line, color=color)
...
ani.save(svar.den_path+'movies/trans.mp4', writer=writer)
