# -*- coding: utf-8 -*-
"""
Created on Tue Jun 18 12:08:36 2019
...
for d in range(svar.dumps):
    vel = svar.v_scl * myFile['{0}/Velocity/time_{1}'.format(svar.fluid, d)][0::, 0::, vel_z_ind]
    vel_long_spline_x = sci.RectBivariateSpline(x_dom_og, x_dom_og, vel[0::, 0::, 0])
    vel_long_spline_y = sci.RectBivariateSpline(x_dom_og, x_dom_og, vel[0::, 0::, 1])
    for x_ind, x in enumerate(x_dom):
        for y_ind, y in enumerate(x_dom):
            vel_x = vel_long_spline_x(x, y)
            vel_y = vel_long_spline_y(x, y)
            vl = np.array([vel_x, vel_y])
            
            vel_long_rad[d, x_ind, y_ind] = radial_vel(vl, x, y)
...
for d in range(svar.dumps):
    ...
    ln = vel_long_rad[d, half_pts, 0::]    
    im = vel_long_rad[d]
    
    image = ax1.imshow(im, extent=(x_dom[0], x_dom[-1], x_dom[0], x_dom[-1]), vmin=vel_min, vmax=vel_max, animated=True)
    
    if d == 0:
        line, = ax2.plot(x_dom, ln, color=color, label='line out')
    else:
        line, = ax2.plot(x_dom, ln, color=color)
...
ani.save(svar.vel_path+'movies/trans.mp4', writer=writer)
