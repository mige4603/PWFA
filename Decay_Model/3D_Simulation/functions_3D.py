...
def calculate_density(den, vel_x, vel_y, vel_z, den_sh, Zsize):
    ...
    hv_yBnd = var.hv_den_r * ( (den[2::, 0, 1:-1] - 2*den[1:-1, 0, 1:-1] + den[0:-2, 0, 1:-1]) + (den[1:-1, 2, 1:-1] - 2*den[1:-1, 1, 1:-1] + den[1:-1, 0, 1:-1]) ) + var.hv_den_z * var.size_ratio * (den[1:-1, 0, 2::] - 2*den[1:-1, 0, 1:-1] + den[1:-1, 0, 0:-2])
    hv_yzBnd = var.hv_den_r * ( (den[2::, 0, 0] - 2*den[1:-1, 0, 0] + den[0:-2, 0, 0]) + (den[1:-1, 2, 0] - 2*den[1:-1, 1, 0] + den[1:-1, 0, 0])) + var.hv_den_z * var.size_ratio * (den[1:-1, 0, 2] - 2*den[1:-1, 0, 1] + den[1:-1, 0, 0])
    ...
    hv_xBnd = var.hv_den_r * ( (den[2, 1:-1, 1:-1] - 2*den[1, 1:-1, 1:-1] + den[0, 1:-1, 1:-1]) + (den[0, 2::, 1:-1] - 2*den[0, 1:-1, 1:-1] + den[0, 0:-2, 1:-1]) ) + var.hv_den_z * var.size_ratio * (den[0, 1:-1, 2::] - 2*den[0, 1:-1, 1:-1] + den[0, 1:-1, 0:-2])
    hv_xzBnd = var.hv_den_r * ( (den[2, 1:-1, 0] - 2*den[1, 1:-1, 0] + den[0, 1:-1, 0]) + (den[0, 2::, 0] - 2*den[0, 1:-1, 0] + den[0, 0:-2, 0]) ) + var.hv_den_z * var.size_ratio * (den[0, 1:-1, 2] - 2*den[0, 1:-1, 1] + den[0, 1:-1, 0])
    
    hv_xBnd = np.insert(hv_xBnd, 0, hv_xzBnd, axis=1)
    hv_xBnd = np.append(hv_xBnd, hv_xzBnd.reshape(var.Rpts-2, 1), axis=1)
    
    hv_xyBnd = var.hv_den_r * ( (den[2, 0, 1:-1] - 2*den[1, 0, 1:-1] + den[0, 0, 1:-1]) + (den[0, 2, 1:-1] - 2*den[0, 1, 1:-1] + den[0, 0, 1:-1]) ) + var.hv_den_z * var.size_ratio * (den[0, 0, 2::] - 2*den[0, 0, 1:-1] + den[0, 0, 0:-2])
    hv_xyzCrn = var.hv_den_r * ( (den[2,0,0] - 2*den[1,0,0] + den[0,0,0]) + (den[0,2,0] - 2*den[0,1,0] + den[0,0,0]) ) + var.hv_den_z * var.size_ratio * (den[0,0,2] - 2*den[0,0,1] + den[0,0,0])
    ...
    den_yBnd = vel_x[1:-1, 0, 1:-1] * (den[2::, 0, 1:-1] - den[0:-2, 0, 1:-1]) + vel_y[1:-1, 0, 1:-1] * (den[1:-1, 1, 1:-1] - den[1:-1, 0, 1:-1]) + var.size_ratio * vel_z[1:-1, 0, 1:-1] * (den[1:-1, 0, 2::] - den[1:-1, 0, 0:-2]) + den[1:-1, 0, 1:-1] * (vel_x[2::, 0, 1:-1] - vel_x[0:-2, 0, 1:-1] + vel_y[1:-1, 1, 1:-1] - vel_y[1:-1, 0, 1:-1] + var.size_ratio * (vel_z[1:-1, 0, 2::] - vel_z[1:-1, 0, 0:-2]) )
    den_yzBnd = vel_x[1:-1, 0, 0] * (den[2::, 0, 0] - den[0:-2, 0, 0]) + vel_y[1:-1, 0, 0] * (den[1:-1, 1, 0] - den[1:-1, 0, 0]) + var.size_ratio * vel_z[1:-1, 0, 0] * (den[1:-1, 0, 1] - den[1:-1, 0, 0]) + den[1:-1, 0, 0] * (vel_x[2::, 0, 0] - vel_x[0:-2, 0, 0] + vel_y[1:-1, 1, 0] - vel_y[1:-1, 0, 0] + var.size_ratio * (vel_z[1:-1, 0, 1] - vel_z[1:-1, 0, 0]) )
    ...
    den_xBnd = vel_x[0, 1:-1, 1:-1] * (den[1, 1:-1, 1:-1] - den[0, 1:-1, 1:-1]) + vel_y[0, 1:-1, 1:-1] * (den[0, 2::, 1:-1] - den[0, 0:-2, 1:-1]) + var.size_ratio * vel_z[0, 1:-1, 1:-1] * (den[0, 1:-1, 2::] - den[0, 1:-1, 0:-2]) + den[0, 1:-1, 1:-1] * (vel_x[1, 1:-1, 1:-1] - vel_x[0, 1:-1, 1:-1] + vel_y[0, 2::, 1:-1] - vel_y[0, 0:-2, 1:-1] + var.size_ratio * (vel_z[0, 1:-1, 2::] - vel_z[0, 1:-1, 0:-2]) )
    den_xzBnd = vel_x[0, 1:-1, 0] * (den[1, 1:-1, 1] - den[0, 1:-1, 0]) + vel_y[0, 1:-1, 0] * (den[0, 2::, 0] - den[0, 0:-2, 0]) + var.size_ratio * vel_z[0, 1:-1, 0] * (den[0, 1:-1, 1] - den[0, 1:-1, 0]) + den[0, 1:-1, 0] * (vel_x[1, 1:-1, 0] - vel_x[0, 1:-1, 0] + vel_y[0, 2::, 0] - vel_y[0, 0:-2, 0] + var.size_ratio * (vel_z[0, 1:-1, 1] - vel_z[0, 1:-1, 0]) )

    den_xBnd = np.insert(den_xBnd, 0, den_xzBnd, axis=1)
    den_xBnd = np.append(den_xBnd, den_xzBnd.reshape(var.Rpts-2, 1), axis=1)

    den_xyBnd = vel_x[0, 0, 1:-1] * (den[1, 0, 1:-1] - den[0, 0, 1:-1]) + vel_y[0, 0, 1:-1] * (den[0, 1, 1:-1] - den[0, 0, 1:-1]) + var.size_ratio * vel_z[0, 0, 1:-1] * (den[0, 0, 2::] - den[0, 0, 0:-2]) + den[0, 0, 1:-1] * (vel_x[1, 0, 1:-1] - vel_x[0, 0, 1:-1] + vel_y[0, 1, 1:-1] - vel_y[0, 0, 1:-1] + var.size_ratio * (vel_z[0, 0, 2::] - vel_z[0, 0, 0:-2]) )
    den_xyzCrn = vel_x[0,0,0] * (den[1,0,0] - den[0,0,0]) + vel_y[0,0,0] * (den[0,1,0] - den[0,0,0]) + var.size_ratio * vel_z[0,0,0] * (den[0,0,1] - den[0,0,0]) + den[0,0,0] * (vel_x[1,0,0] - vel_x[0,0,0] + vel_y[0,1,0] - vel_y[0,0,0] + var.size_ratio * (vel_z[0,0,1] - vel_z[0,0,0]) )
...
def calculate_velocity_x(vel_x, vel_y, vel_z, den, den_inv, vel_sh, k, Zsize):
    ...
    hv_yBnd = var.hv_vel_r * ( (vel_x[2::, 0, 1:-1] - 2*vel_x[1:-1, 0, 1:-1] + vel_x[0:-2, 0, 1:-1]) + (vel_x[1:-1, 2, 1:-1] - 2*vel_x[1:-1, 1, 1:-1] + vel_x[1:-1, 0, 1:-1]) ) + var.hv_vel_z * var.size_ratio * (vel_x[1:-1, 0, 2::] - 2*vel_x[1:-1, 0, 1:-1] + vel_x[1:-1, 0, 0:-2])
    hv_yzBnd = var.hv_vel_r * ( (vel_x[2::, 0, 0] - 2*vel_x[1:-1, 0, 0] + vel_x[0:-2, 0, 0]) + (vel_x[1:-1, 2, 0] - 2*vel_x[1:-1, 1, 0] + vel_x[1:-1, 0, 0]) ) + var.hv_vel_z * var.size_ratio * (vel_x[1:-1, 0, 2] - 2*vel_x[1:-1, 0, 1] + vel_x[1:-1, 0, 0]) 
    ...
    hv_xBnd = var.hv_vel_r * ( (vel_x[2, 1:-1, 1:-1] - 2*vel_x[1, 1:-1, 1:-1] + vel_x[0, 1:-1, 1:-1]) + (vel_x[0, 2::, 1:-1] - 2*vel_x[0, 1:-1, 1:-1] + vel_x[0, 0:-2, 1:-1]) ) + var.hv_vel_z * var.size_ratio * (vel_x[0, 1:-1, 2::] - 2*vel_x[0, 1:-1, 1:-1] + vel_x[0, 1:-1, 0:-2])
    hv_xzBnd = var.hv_vel_r * ( (vel_x[2, 1:-1, 0] - 2*vel_x[1, 1:-1, 0] + vel_x[0, 1:-1, 0]) + (vel_x[0, 2::, 0] - 2*vel_x[0, 1:-1, 0] + vel_x[0, 0:-2, 0]) ) + var.hv_vel_z * var.size_ratio * (vel_x[0, 1:-1, 2] - 2*vel_x[0, 1:-1, 1] + vel_x[0, 1:-1, 0]) 
    
    hv_xBnd = np.insert(hv_xBnd, 0, hv_xzBnd, axis=1)
    hv_xBnd = np.append(hv_xBnd, hv_xzBnd.reshape(var.Rpts-2, 1), axis=1)
    
    hv_xyBnd = var.hv_vel_r * ( (vel_x[2, 0, 1:-1] - 2*vel_x[1, 0, 1:-1] + vel_x[0, 0, 1:-1]) + (vel_x[0, 2, 1:-1] - 2*vel_x[0, 1, 1:-1] + vel_x[0, 0, 1:-1]) ) + var.hv_vel_z * var.size_ratio * (vel_x[0, 0, 2::] - 2*vel_x[0, 0, 1:-1] + vel_x[0, 0, 0:-2])
    hv_xyzCrn = var.hv_vel_r * ( (vel_x[2,0,0] - 2*vel_x[1,0,0] + vel_x[0,0,0]) + (vel_x[0,2,0] - 2*vel_x[0,1,0] + vel_x[0,0,0]) ) + var.hv_vel_z * var.size_ratio * (vel_x[0,0,2] - 2*vel_x[0,0,1] + vel_x[0,0,0])
    ...
    vel_x_yBnd = vel_x[1:-1, 0, 1:-1] * (vel_x[2::, 0, 1:-1] - vel_x[0:-2, 0, 1:-1]) + vel_y[1:-1, 0, 1:-1] * (vel_x[1:-1, 1, 1:-1] - vel_x[1:-1, 0, 1:-1]) + var.size_ratio * vel_z[1:-1, 0, 1:-1] * (vel_x[1:-1, 0, 2::] - vel_x[1:-1, 0, 0:-2]) + 2 * k * den_inv[1:-1, 0, 1:-1] * (den[2::, 0, 1:-1] - den[0:-2, 0, 1:-1])
    vel_x_yzBnd = vel_x[1:-1, 0, 0] * (vel_x[2::, 0, 0] - vel_x[0:-2, 0, 0]) + vel_y[1:-1, 0, 0] * (vel_x[1:-1, 1, 0] - vel_x[1:-1, 0, 0]) + var.size_ratio * vel_z[1:-1, 0, 0] * (vel_x[1:-1, 0, 1] - vel_x[1:-1, 0, 0]) + 2 * k * den_inv[1:-1, 0, 0] * (den[2::, 0, 0] - den[0:-2, 0, 0])
    ...
    vel_x_xBnd = vel_x[0, 1:-1, 1:-1] * (vel_x[1, 1:-1, 1:-1] - vel_x[0, 1:-1, 1:-1]) + vel_y[0, 1:-1, 1:-1] * (vel_x[0, 2::, 1:-1] - vel_x[0, 0:-2, 1:-1]) + var.size_ratio * vel_z[0, 1:-1, 1:-1] * (vel_x[0, 1:-1, 2::] - vel_x[0, 1:-1, 0:-2]) + 2 * k * den_inv[0, 1:-1, 1:-1] * (den[1, 1:-1, 1:-1] - den[0, 1:-1, 1:-1])
    vel_x_xzBnd = vel_x[0, 1:-1, 0] * (vel_x[1, 1:-1, 0] - vel_x[0, 1:-1, 0]) + vel_y[0, 1:-1, 0] * (vel_x[0, 2::, 0] - vel_x[0, 0:-2, 0]) + var.size_ratio * vel_z[0, 1:-1, 0] * (vel_x[0, 1:-1, 1] - vel_x[0, 1:-1, 0]) + 2 * k * den_inv[0, 1:-1, 0] * (den[1, 1:-1, 0] - den[0, 1:-1, 0])    
    ...
    vel_x_xyBnd = vel_x[0, 0, 1:-1] * (vel_x[1, 0, 1:-1] - vel_x[0, 0, 1:-1]) + vel_y[0, 0, 1:-1] * (vel_x[0, 1, 1:-1] - vel_x[0, 0, 1:-1]) + var.size_ratio * vel_z[0, 0, 1:-1] * (vel_x[0, 0, 2::] - vel_x[0, 0, 0:-2]) + 2 * k * den_inv[0, 0, 1:-1] * (den[1, 0, 1:-1] - den[0, 0, 1:-1])
    vel_x_xyzCrn = vel_x[0,0,0] * (vel_x[1,0,0] - vel_x[0,0,0]) + vel_y[0,0,0] * (vel_x[0,1,0] - vel_x[0,0,0]) + var.size_ratio * vel_z[0,0,0] * (vel_x[0,0,1] - vel_x[0,0,0]) + 2 * k * den_inv[0,0,0] * (den[1,0,0] - den[0,0,0])
...
def calculate_velocity_y(vel_x, vel_y, vel_z, den, den_inv, vel_sh, k, Zsize):
    ...
    hv_yBnd = var.hv_vel_r * ( (vel_y[2::, 0, 1:-1] - 2*vel_y[1:-1, 0, 1:-1] + vel_y[0:-2, 0, 1:-1]) + (vel_y[1:-1, 2, 1:-1] - 2*vel_y[1:-1, 1, 1:-1] + vel_y[1:-1, 0, 1:-1]) ) + var.hv_vel_z * var.size_ratio * (vel_y[1:-1, 0, 2::] - 2*vel_y[1:-1, 0, 1:-1] + vel_y[1:-1, 0, 0:-2])
    hv_yzBnd = var.hv_vel_r * ( (vel_y[2::, 0, 0] - 2*vel_y[1:-1, 0, 0] + vel_y[0:-2, 0, 0]) + (vel_y[1:-1, 2, 0] - 2*vel_y[1:-1, 1, 0] + vel_y[1:-1, 0, 0]) ) + var.hv_vel_z * var.size_ratio * (vel_y[1:-1, 0, 2] - 2*vel_y[1:-1, 0, 1] + vel_y[1:-1, 0, 0])
    ...
    hv_xBnd = var.hv_vel_r * ( (vel_y[2, 1:-1, 1:-1] - 2*vel_y[1, 1:-1, 1:-1] + vel_y[0, 1:-1, 1:-1]) + (vel_y[0, 2::, 1:-1] - 2*vel_y[0, 1:-1, 1:-1] + vel_y[0, 0:-2, 1:-1]) ) + var.hv_vel_z * var.size_ratio * (vel_y[0, 1:-1, 2::] - 2*vel_y[0, 1:-1, 1:-1] + vel_y[0, 1:-1, 0:-2])
    hv_xzBnd = var.hv_vel_r * ( (vel_y[2, 1:-1, 0] - 2*vel_y[1, 1:-1, 0] + vel_y[0, 1:-1, 0]) + (vel_y[0, 2::, 0] - 2*vel_y[0, 1:-1, 0] + vel_y[0, 0:-2, 0]) ) + var.hv_vel_z * var.size_ratio * (vel_y[0, 1:-1, 2] - 2*vel_y[0, 1:-1, 1] + vel_y[0, 1:-1, 0])
    
    hv_xBnd = np.insert(hv_xBnd, 0, hv_xzBnd, axis=1)
    hv_xBnd = np.append(hv_xBnd, hv_xzBnd.reshape(var.Rpts-2, 1), axis=1)
    
    hv_xyBnd = var.hv_vel_r * ( (vel_y[2, 0, 1:-1] - 2*vel_y[1, 0, 1:-1] + vel_y[0, 0, 1:-1]) + (vel_y[0, 2, 1:-1] - 2*vel_y[0, 1, 1:-1] + vel_y[0, 0, 1:-1]) ) + var.hv_vel_z * var.size_ratio * (vel_y[0, 0, 2::] - 2*vel_y[0, 0, 1:-1] + vel_y[0, 0, 0:-2])
    hv_xyzCrn = var.hv_vel_r * ( (vel_y[2,0,0] - 2*vel_y[1,0,0] + vel_y[0,0,0]) + (vel_y[0,2,0] - 2*vel_y[0,1,0] + vel_y[0,0,0]) ) + var.hv_vel_z * var.size_ratio * (vel_y[0,0,2] - 2*vel_y[0,0,1] + vel_y[0,0,0])
    ...
    vel_y_yBnd = vel_x[1:-1, 0, 1:-1] * (vel_y[2::, 0, 1:-1] - vel_y[0:-2, 0, 1:-1]) + vel_y[1:-1, 0, 1:-1] * (vel_y[1:-1, 1, 1:-1] - vel_y[1:-1, 0, 1:-1]) + var.size_ratio * vel_z[1:-1, 0, 1:-1] * (vel_y[1:-1, 0, 2::] - vel_y[1:-1, 0, 0:-2]) + 2 * k * den_inv[1:-1, 0, 1:-1] * (den[1:-1, 1, 1:-1] - den[1:-1, 0, 1:-1])
    vel_y_yzBnd = vel_x[1:-1, 0, 0] * (vel_y[2::, 0, 0] - vel_y[0:-2, 0, 0]) + vel_y[1:-1, 0, 0] * (vel_y[1:-1, 1, 0] - vel_y[1:-1, 0, 0]) + var.size_ratio * vel_z[1:-1, 0, 0] * (vel_y[1:-1, 0, 1] - vel_y[1:-1, 0, 0]) + 2 * k * den_inv[1:-1, 0, 0] * (den[1:-1, 1, 0] - den[1:-1, 0, 0])
    ...
    vel_y_xBnd = vel_x[0, 1:-1, 1:-1] * (vel_y[1, 1:-1, 1:-1] - vel_y[0, 1:-1, 1:-1]) + vel_y[0, 1:-1, 1:-1] * (vel_y[0, 2::, 1:-1] - vel_y[0, 0:-2, 1:-1]) + var.size_ratio * vel_z[0, 1:-1, 1:-1] * (vel_y[0, 1:-1, 2::] - vel_y[0, 1:-1, 0:-2]) + 2 * k * den_inv[0, 1:-1, 1:-1] * (den[0, 2::, 1:-1] - den[0, 0:-2, 1:-1])
    vel_y_xzBnd = vel_x[0, 1:-1, 0] * (vel_y[1, 1:-1, 0] - vel_y[0, 1:-1, 0]) + vel_y[0, 1:-1, 0] * (vel_y[0, 2::, 0] - vel_y[0, 0:-2, 0]) + var.size_ratio * vel_z[0, 1:-1, 0] * (vel_y[0, 1:-1, 1] - vel_y[0, 1:-1, 0]) + 2 * k * den_inv[0, 1:-1, 0] * (den[0, 2::, 0] - den[0, 0:-2, 0])
    ...
    vel_y_xyBnd = vel_x[0, 0, 1:-1] * (vel_y[1, 0, 1:-1] - vel_y[0, 0, 1:-1]) + vel_y[0, 0, 1:-1] * (vel_y[0, 1, 1:-1] - vel_y[0, 0, 1:-1]) + var.size_ratio * vel_z[0, 0, 1:-1] * (vel_y[0, 0, 2::] - vel_y[0, 0, 0:-2]) + 2 * k * den_inv[0, 0, 1:-1] * (den[0, 1, 1:-1] - den[0, 0, 1:-1])
    vel_y_xyzCrn = vel_x[0,0,0] * (vel_y[1,0,0] - vel_y[0,0,0]) + vel_y[0,0,0] * (vel_y[0,1,0] - vel_y[0,0,0]) + var.size_ratio * vel_z[0,0,0] * (vel_y[0,0,1] - vel_y[0,0,0]) + 2 * k * den_inv[0,0,0] * (den[0,1,0] - den[0,0,0])
...
def calculate_velocity_z(vel_x, vel_y, vel_z, den, den_inv, vel_sh, k, Zsize):
    ...
    hv_yBnd = var.hv_vel_r * ( (vel_z[2::, 0, 1:-1] - 2*vel_z[1:-1, 0, 1:-1] + vel_z[0:-2, 0, 1:-1]) + (vel_z[1:-1, 2, 1:-1] - 2*vel_z[1:-1, 1, 1:-1] + vel_z[1:-1, 0, 1:-1]) ) + var.hv_vel_z * var.size_ratio * (vel_z[1:-1, 0, 2::] - 2*vel_z[1:-1, 0, 1:-1] + vel_z[1:-1, 0, 0:-2])
    hv_yzBnd = var.hv_vel_r * ( (vel_z[2::, 0, 0] - 2*vel_z[1:-1, 0, 0] + vel_z[0:-2, 0, 0]) + (vel_z[1:-1, 2, 0] - 2*vel_z[1:-1, 1, 0] + vel_z[1:-1, 0, 0]) ) + var.hv_vel_z * var.size_ratio * (vel_z[1:-1, 0, 2] - 2*vel_z[1:-1, 0, 1] + vel_z[1:-1, 0, 0])
    ...
    hv_xBnd = var.hv_vel_r * ( (vel_z[2, 1:-1, 1:-1] - 2*vel_z[1, 1:-1, 1:-1] + vel_z[0, 1:-1, 1:-1]) + (vel_z[0, 2::, 1:-1] - 2*vel_z[0, 1:-1, 1:-1] + vel_z[0, 0:-2, 1:-1]) ) + var.hv_vel_z * var.size_ratio * (vel_z[0, 1:-1, 2::] - 2*vel_z[0, 1:-1, 1:-1] + vel_z[0, 1:-1, 0:-2])
    hv_xzBnd = var.hv_vel_r * ( (vel_z[2, 1:-1, 0] - 2*vel_z[1, 1:-1, 0] + vel_z[0, 1:-1, 0]) + (vel_z[0, 2::, 0] - 2*vel_z[0, 1:-1, 0] + vel_z[0, 0:-2, 0]) ) + var.hv_vel_z * var.size_ratio * (vel_z[0, 1:-1, 2] - 2*vel_z[0, 1:-1, 1] + vel_z[0, 1:-1, 0])
    
    hv_xBnd = np.insert(hv_xBnd, 0, hv_xzBnd, axis=1)
    hv_xBnd = np.append(hv_xBnd, hv_xzBnd.reshape(var.Rpts-2, 1), axis=1)
    
    hv_xyBnd = var.hv_vel_r * ( (vel_z[2, 0, 1:-1] - 2*vel_z[1, 0, 1:-1] + vel_z[0, 0, 1:-1]) + (vel_z[0, 2, 1:-1] - 2*vel_z[0, 1, 1:-1] + vel_z[0, 0, 1:-1]) ) + var.hv_vel_z * var.size_ratio * (vel_z[0, 0, 2::] - 2*vel_z[0, 0, 1:-1] + vel_z[0, 0, 0:-2])
    hv_xyzCrn = var.hv_vel_r * ( (vel_z[2,0,0] - 2*vel_z[1,0,0] + vel_z[0,0,0]) + (vel_z[0,2,0] - 2*vel_z[0,1,0] + vel_z[0,0,0]) ) + var.hv_vel_z * var.size_ratio * (vel_z[0,0,2] - 2*vel_z[0,0,1] + vel_z[0,0,0])
    ...
    vel_z_yBnd = vel_x[1:-1, 0, 1:-1] * (vel_z[2::, 0, 1:-1] - vel_z[0:-2, 0, 1:-1]) + vel_y[1:-1, 0, 1:-1] * (vel_z[1:-1, 1, 1:-1] - vel_z[1:-1, 0, 1:-1]) + var.size_ratio * vel_z[1:-1, 0, 1:-1] * (vel_z[1:-1, 0, 2::] - vel_z[1:-1, 0, 0:-2]) + var.size_ratio * 2 * k * den_inv[1:-1, 0, 1:-1] * (den[1:-1, 0, 2::] - den[1:-1, 0, 0:-2])
    vel_z_yzBnd = vel_x[1:-1, 0, 0] * (vel_z[2::, 0, 0] - vel_z[0:-2, 0, 0]) + vel_y[1:-1, 0, 0] * (vel_z[1:-1, 1, 0] - vel_z[1:-1, 0, 0]) + var.size_ratio * vel_z[1:-1, 0, 0] * (vel_z[1:-1, 0, 1] - vel_z[1:-1, 0, 0]) + var.size_ratio * 2 * k * den_inv[1:-1, 0, 0] * (den[1:-1, 0, 1] - den[1:-1, 0, 0])
    ...
    vel_z_xBnd = vel_x[0, 1:-1, 1:-1] * (vel_z[1, 1:-1, 1:-1] - vel_z[0, 1:-1, 1:-1]) + vel_y[0, 1:-1, 1:-1] * (vel_z[0, 2::, 1:-1] - vel_z[0, 0:-2, 1:-1]) + var.size_ratio * vel_z[0, 1:-1, 1:-1] * (vel_z[0, 1:-1, 2::] - vel_z[0, 1:-1, 0:-2]) + var.size_ratio * 2 * k * den_inv[0, 1:-1, 1:-1] * (den[0, 1:-1, 2::] - den[0, 1:-1, 0:-2])
    vel_z_xzBnd = vel_x[0, 1:-1, 0] * (vel_z[1, 1:-1, 0] - vel_z[0, 1:-1, 0]) + vel_y[0, 1:-1, 0] * (vel_z[0, 2::, 0] - vel_z[0, 0:-2, 0]) + var.size_ratio * vel_z[0, 1:-1, 0] * (vel_z[0, 1:-1, 1] - vel_z[0, 1:-1, 0]) + var.size_ratio * 2 * k * den_inv[0, 1:-1, 0] * (den[0, 1:-1, 1] - den[0, 1:-1, 0])
    ...
    vel_z_xyBnd = vel_x[0, 0, 1:-1] * (vel_z[1, 0, 1:-1] - vel_z[0, 0, 1:-1]) + vel_y[0, 0, 1:-1] * (vel_z[0, 1, 1:-1] - vel_z[0, 0, 1:-1]) + var.size_ratio * vel_z[0, 0, 1:-1] * (vel_z[0, 0, 2::] - vel_z[0, 0, 0:-2]) + var.size_ratio * 2 * k * den_inv[0, 0, 1:-1] * (den[0, 0, 2::] - den[0, 0, 0:-2])
    vel_z_xyzCrn = vel_x[0,0,0] * (vel_z[1,0,0] - vel_z[0,0,0]) + vel_y[0,0,0] * (vel_z[0,1,0] - vel_z[0,0,0]) + var.size_ratio * vel_z[0,0,0] * (vel_z[0,0,1] - vel_z[0,0,0]) + var.size_ratio * 2 * k * den_inv[0,0,0] * (den[0,0,1] - den[0,0,0])
